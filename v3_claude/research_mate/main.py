...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(log_dir / "researchmate.log"),
...
class MemoryManager:
    ...
    def store_query_result(self, query: str, results: Dict):
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?)
        """, (query_hash, query, json.dumps(serializable_results), datetime.now().isoformat()))
        conn.commit()
        conn.close()
...
@app.get("/research/markdown/{job_id}")
async def get_results_markdown(job_id: str):
    """Get job results formatted as markdown"""
    ...
    if job_id in agent.active_jobs:
        ...
        conn = sqlite3.connect(config.DATABASE_PATH)
        cursor = conn.execute(
            "SELECT status, results FROM research_jobs WHERE job_id = ?",
            (job_id,)
        )
        row = cursor.fetchone()
        conn.close()
...
def generate_markdown_report(results: Dict) -> str:
    ...
    # Header
    md.append(f"# Research Report: {results['query']}")
    md.append(f"*Generated on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
    md.append("")

    # Summary
    md.append("## Summary")
    md.append(f"- **Papers Found:** {results['papers_found']}")
    md.append(f"- **Processing Time:** {results['processing_time']:.1f}s")
    md.append("")

    # Synthesis
    if results.get('synthesis'):
        md.append("## Key Insights")
        md.append(results['synthesis'])
        md.append("")

    # Papers
    md.append("## Papers Analyzed")
    md.append("")

    for i, paper in enumerate(results['papers'], 1):
        md.append(f"### {i}. {paper['title']}")
        md.append(f"**Authors:** {', '.join(paper['authors'])}")

        if paper.get('venue'):
            md.append(f"**Venue:** {paper['venue']}")
        if paper.get('published'):
            md.append(f"**Published:** {paper['published']}")
        if paper.get('citation_count'):
            md.append(f"**Citations:** {paper['citation_count']}")
        if paper.get('url'):
            md.append(f"**URL:** {paper['url']}")

        md.append("")
        md.append("**Analysis:**")
        md.append(paper['analysis'])
        md.append("")
        md.append("---")
        md.append("")
    ...
    if results.get('classifications'):
        md.append("## Classifications")
        md.append("")
        for paper_key, classification in results['classifications'].items():
            paper_num = paper_key.split('_')[1]
            md.append(f"**Paper {paper_num}:** {classification['category']} (confidence: {classification['confidence']:.1f})")
        md.append("")

    # Footer
    md.append("---")
    md.append("*Generated by ResearchMate Agent*")
